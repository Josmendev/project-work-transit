...
export interface TableZoneItemProps {
  listOfZones: Array<ZoneResponse>;
  editRow?: (data: Zone) => void;
  deleteRow?: (data: Zone) => void;
  activateRow?: (data: Zone) => void;
  currentPage: number;
}

export const TableZoneItem: React.FC<TableZoneItemProps> = ({
  listOfZones,
  editRow,
  deleteRow,
  activateRow,
  ...
  return (
    <>
      {listOfZones.map((row, rowIndex) => (
        <tr key={row?.zoneId} className="border-b border-neutral-100 text-center">
