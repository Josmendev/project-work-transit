...
import type { ConnectionPoint, ConnectionPointResponse } from "../types/ConnectionPoint";

export interface TableConnectionPointItemProps {
  listOfConnectionPoints: Array<ConnectionPointResponse>;
  editRow?: (data: ConnectionPoint) => void;
  deleteRow?: (data: ConnectionPoint) => void;
  activateRow?: (data: ConnectionPoint) => void;
  currentPage: number;
}

export const TableConnectionPointItem: React.FC<TableConnectionPointItemProps> = ({
  listOfConnectionPoints,
  editRow,
  deleteRow,
  activateRow,
  currentPage,
  ...
  return (
    <>
      {listOfConnectionPoints.map((row, rowIndex) => (
        <tr key={row?.connectionPointId} className="border-b border-neutral-100 text-center">
          ...
          <td className="p-3 w-[180px] flex gap-1.5 items-center justify-center">
            {editRow && (
              <Button
                title="Editar"
                id={`btnEditRow-${row?.connectionPointId}`}
                aria-data={row}
            ...
            {row.isActive && deleteRow && (
              <Button
                title="Eliminar"
                id={`btnDeleteRow-${row?.connectionPointId}`}
                aria-data={row}
            ...
            {!row.isActive && activateRow && (
              <Button
                title="Activar"
                id={`btnactivateRow-${row?.connectionPointId}`}
                aria-data={row}
