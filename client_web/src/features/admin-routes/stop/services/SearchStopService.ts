...
export const SearchStopService = async ({
  limit = LIMIT_PAGE,
  page = INITIAL_PAGE,
  query = "",
}: {
  ...
  page: number;
  query: string;
}): Promise<DataResponseFromAPI<StopResponse>> => {
  try {
    const { token } = JSON.parse(sessionStorage.getItem("user") as string);
    if (!token) throw new Error("Token inválido");

    const response = await fetch(`${ENDPOINT_STOP}/${query}?limit=${limit}&page=${page}`, {
      method: "GET",
      headers: {
    ...
    // Respuesta exitosa
    const data: DataResponseFromAPI<StopResponse> = await response.json();
    return data;
  } catch (error: unknown) {
